                ...
                case 'staff':
                    ...
                    case 'prisoners':
                        $data = Prisoner::where('prison_id', $prisonId)
                            ->get()
                            ->map(function ($prisoner) {
                                $start = $prisoner->time_serve_start;
                                $end = $prisoner->time_serve_end;
                    
                                // Check if both start and end are valid dates
                                $validDates = strtotime($start) !== false && strtotime($end) !== false;
                    
                                return [
                                    'id' => $prisoner->id,
                                    'name' => trim("{$prisoner->first_name} {$prisoner->middle_name} {$prisoner->last_name}"),
                                    'prison' => $prisoner->prison->name ?? 'Unknown',
                                    'sentence' => $validDates
                                        ? Carbon::parse($start)->diff(Carbon::parse($end))->format('%y years, %m months, %d days')
                                        : ($prisoner->sentence ?? 'Unknown'),
                                    'status' => $prisoner->status ?? 'Incarcerated'
                                ];
                            });
                        break;
